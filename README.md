...
- **模块定位**：构建树形节点流程，适合菜单和分层流程。
- **核心入口**：`LogicNodeManager`、`LogicNodeTreeAsset`、`LogicNodeMono`
- **使用方法**：
  1. 创建 `LogicNodeTreeAsset` 并配置父子层级关系。
  2. 使用控制节点处理激活、回退和跳转逻辑。
  3. 将 UI 页签/面板状态映射到树节点状态机。
...
| AssetBundle 辅助工具 | 打包选项配置、平台切换、批量设置包名 | `NonsensicalKit/AssestBundle辅助工具` |
| Updater 编辑器 | 更新流程相关配置的可视化编辑与调试 | `UpdaterEditor` 窗口入口 |
| DAG 可视化编辑器 | 以图编辑方式配置 DAG 节点与连接关系 | `DagEditorWindow` |
| 逻辑树编辑器 | 树节点的层级编辑、增删与折叠展示 | `LogicNodeTreeAssetEditor` / `NodeTreeEdit` |
